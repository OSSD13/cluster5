...
class MapController extends Controller
{
    ...
    {
        $latitude = $request->input('lat', 13.7563);     // fallback to Bangkok lat
        $longitude = $request->input('lng', 100.5018);   // fallback to Bangkok lng
        $radiusInMeters = $request->input('radius', 1000);
        $limitPerType = $request->input('limit', 5);
    ...
    ROW_NUMBER() OVER (
        ...
        $results = DB::table(DB::raw("({$subquery->toSql()}) as poi_sub"))
