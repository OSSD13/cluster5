...
<form method="POST" action="{{ route('api.poi.create') }}" name="poiForm" id="poiForm" autocomplete="off">
    @csrf
    <div class="max-w-md mx-auto bg-white shadow-lg rounded-lg p-6">
        ...
        <select class="w-full p-2 border border-gray-300 rounded-lg mb-3" name="type" id="type">
            ...
            @foreach ($poiTypes as $type)
                <option value="{{ $type->poit_type }}" {{ old('type') == $type->poit_type ? 'selected' : '' }}>
        ...
        <div class="flex justify-between">
            <a href="{{ route('poi.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-lg text-center">ยกเลิก</a>
