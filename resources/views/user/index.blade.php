...
<script>
    ...
    function renderTable(filteredData = null) {
        ...
        async function fetchUsers(search = '') {
            try {
                const res = await fetch(`{{ route('api.user.query') }}?limit=${rowsPerPage}&page=${currentPage}&search=${encodeURIComponent(search)}`);
                const result = await res.json();

                if (!res.ok) {
                    Swal.fire("ผิดพลาด", result.message ?? "โหลดข้อมูลไม่สำเร็จ", "error");
                    return;
                }
    ...
    function formatThaiDate(dateStr) {
        ...
        function renderPagination(totalItems) {
            ...
            const prevBtn = document.createElement("button");
            prevBtn.innerHTML = "&lt;";
            prevBtn.className = `min-w-[40px] h-10 px-3 mx-1 rounded-lg text-xl font-bold
                ${currentPage === 1 ? "text-gray-300 bg-white border border-gray-200 cursor-not-allowed" : "text-blue-600 bg-white border border-gray-300 hover:bg-blue-50"}`;
            prevBtn.disabled = currentPage === 1;
            prevBtn.onclick = () => goToPage(currentPage - 1);
            ...
            const nextBtn = document.createElement("button");
            nextBtn.innerHTML = "&gt;";
            nextBtn.className = `min-w-[40px] h-10 px-3 mx-1 rounded-lg text-xl font-bold
                ${currentPage === totalPages ? "text-gray-300 bg-white border border-gray-200 cursor-not-allowed" : "text-blue-600 bg-white border border-gray-300 hover:bg-blue-50"}`;
            nextBtn.disabled = currentPage === totalPages;
            nextBtn.onclick = () => goToPage(currentPage + 1);
                ...
                btn.onclick = () => {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            goToPage(parseInt(result.value));
                        }
                    });
                };
                return btn;
            ...
            function createPageButton(page, isActive = false) {
                const btn = document.createElement("button");
                btn.innerText = page;
                btn.className = `min-w-[36px] h-10 px-3 mx-1 rounded-lg text-sm font-medium
                    ${isActive ? "bg-blue-600 text-white" : "bg-white border border-gray-300 text-black hover:bg-gray-100"}`;
        ...
        function goToPage(pageNumber) {
        currentPage = pageNumber;
        fetchUsers(document.getElementById("searchInput").value); // โหลดข้อมูลใหม่จาก API พร้อม search ปัจจุบัน
        }
        ...
        function filterAll() {
            ...
            fetchUsersWithUrl(url);
        }

        async function fetchUsersWithUrl(url) {
            try {
                ...
                document.getElementById("resultCount").innerText = `ผลลัพธ์ ${totalMembers} รายการ`;
                renderTable();
                renderPagination(totalMembers);
                populateSupervisorDropdown(); // รีเฟรช dropdown
            } catch (error) {
                console.error(error);
